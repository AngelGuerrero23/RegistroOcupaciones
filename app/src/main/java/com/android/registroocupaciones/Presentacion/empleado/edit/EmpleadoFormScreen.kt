...
fun EmpleadoFormScreen(
    viewModel: EmpleadoFormViewModel = hiltViewModel(), onBack: () -> Unit
) {
    ...
    Scaffold(
        ...
        Column(
            ...
            OutlinedTextField(
                value = state.nombres,
                onValueChange = {
                    viewModel.onEvent(EmpleadoFormUiEvent.NombresChanged(it))
                },
                label = { Text("Nombres") },
                modifier = Modifier
                    .fillMaxWidth()
            ...
            ExposedDropdownMenuBox(
                expanded = expandedSexo, onExpandedChange = { expandedSexo = it }) {
                OutlinedTextField(
                    value = state.sexo,
                    onValueChange = { },
                    readOnly = true,
                ...
                ExposedDropdownMenu(
                    expanded = expandedSexo, onDismissRequest = { expandedSexo = false }) {
            ...
            OutlinedTextField(
                ...
                readOnly = true,
                trailingIcon = {
                    IconButton(onClick = { showDataPicker = true }) {
                        Icon(
            ...
            if (showDataPicker) {
                DatePickerDialog(onDismissRequest = { showDataPicker = false }, confirmButton = {
                    TextButton(
                        onClick = {
                            datePickerState.selectedDateMillis?.let { millis ->
                                val date = Instant.ofEpochMilli(millis).atZone(ZoneId.of("UTC"))
                                    .toLocalDate()
                                viewModel.onEvent(EmpleadoFormUiEvent.fechaChanged(date))
                            }
            ...
            OutlinedTextField(
                value = state.sueldo,
                onValueChange = {
                    viewModel.onEvent(EmpleadoFormUiEvent.sueldoChanged(it))
                },
                label = { Text("Sueldo (en pesos dominicanos)") },
                modifier = Modifier
                    .fillMaxWidth()
                    .testTag("input_sueldo"),
                isError = state.sueldoError != null, // Corregido: antes tenías .let directo causando problemas de tipado
